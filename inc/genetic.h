...
public:
	...
	virtual ResultMH<tDomain> optimize(Problem<tDomain> &problem, int maxevals) override
	{
		...
		vector<tSolution<tDomain>> poblacion(this->tam_poblacion);
		vector<double> fitness_pob(this->tam_poblacion);
		int evaluaciones = 0;
		
		int mejor_idx_historico = 0;
		
		// inicialización de la población
		for (int i = 0; i < this->tam_poblacion && evaluaciones < maxevals; i++)
		{
		...
		while (evaluaciones < maxevals)
		{
			...
			for (int i = 0; i < this->tam_poblacion && evaluaciones < maxevals; i += 2) 
			{
				...
				// mutacion
				MutacionUniforme(h1, this->prob_mutacion, min_val, max_val);
				MutacionUniforme(h2, this->prob_mutacion, min_val, max_val);
...
public:
	...
	virtual ResultMH<tDomain> optimize(Problem<tDomain> &problem, int maxevals) override
	{
		...
		while (evaluaciones < maxevals)
		{
			...
			for (int i = 2; i < this->tam_poblacion; i++)
			{
