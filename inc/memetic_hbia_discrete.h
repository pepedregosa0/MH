/*
Archivo: memetic_hbia_discrete.h
Autor: josepc
Descripcion: Implementacion de la clase MemeticHBIADiscrete que representa
...
class MemeticHBIADiscrete : public MH<int>
{
private:
	struct Bird {
		tSolution<int> sol;
...
public:
	MemeticHBIADiscrete() : MH<int>() {}
	virtual ~MemeticHBIADiscrete() {}
	...
	ResultMH<int> optimize(Problem<int> &problem, int maxevals) override
	{
		...
		while (evals < maxevals)
		{
			...
			// FASE 3: REORGANIZATION Y MEMÉTICA
			...
			if (estancamiento >= theta_estancamiento)
			{
				...
				// Si la BL ha logrado romper el estancamiento perforando hacia un nuevo óptimo:
				if (best_bird.fitness < old_best_fit)
				{
					estancamiento = 0;
					// Actualizamos el pájaro de la bandada con esta solución para no perderla
					banda[0] = best_bird; 
				}

				// Aplicamos la muerte estocástica a los peores
